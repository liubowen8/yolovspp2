...
def compute_loss(p, targets, model):  # predictions, targets, model
    ...
    for i, pi in enumerate(p):  # layer index, layer predictions
        ...
        if nb:
            ...
            sigmas=ps[:, -4:].exp().clamp(min=0.001, max=1E3)
            # pbox = torch.cat((pxy, pwh), 1)  # predicted box
            pbox = torch.cat((pxy, pwh,sigmas), 1)  # predicted box
            ...
            下面注释的是 高斯loss
            """
            # mll=maximum_likelihood_loss(pbox.t(), tbox[i], x1y1x2y2=False)
            # lbox+=mll.mean()
            # print(ps[:, -4:][0])
            # print(sigmas[0], 111111)
            # print(mll.mean(), 2222222)
            ...
            if model.nc > 1:  # cls loss (only if multiple classes)
                t = torch.full_like(ps[:, 5:-4], cn, device=device)  # targets
                t[range(nb), tcls[i]] = cp
                lcls += BCEcls(ps[:, 5:-4], t)  # BCE
...
def non_max_suppression(prediction, conf_thres=0.1, iou_thres=0.6,
    ...
    for xi, x in enumerate(prediction):  # image index, image inference 遍历每张图片
        ...
        if multi_label:  # 针对每个类别执行非极大值抑制,只要是大于阈值的类别都进行NMS
            i, j = (x[:, 5:-4] > conf_thres).nonzero(as_tuple=False).t()
            #x = torch.cat((box[i], x[i, j + 5].unsqueeze(1), j.float().unsqueeze(1)), 1)
            # x = torch.cat((box[i], x[i, j + 5].unsqueeze(1), j.float().unsqueeze(1),sigmas[i]), 1)
            # 把类别分数也添加进来
            x = torch.cat((box[i], x[i, j + 5].unsqueeze(1), j.float().unsqueeze(1), class_score[i] ,sigmas[i]), 1)
            # 这里的 前四个维度表示边界框，接着的两个维度表示 ，概率中最大的那个类别的概率和它对应的类别。
        else:  # best class only  只针对每个类别中概率最大的类别进行非极大值抑制处理
            conf, j = x[:, 5:-4].max(1)
