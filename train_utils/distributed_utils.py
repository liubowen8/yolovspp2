...
def init_distributed_mode(args):
    """
    print(os.environ,5655555555)
    'MASTER_ADDR': '127.0.0.1', 'MASTER_PORT': '29500', 'WORLD_SIZE': '8', 'OMP_NUM_THREADS': '1', 'RANK': '7', 'LOCAL_RANK': '7'
    """
    if 'RANK' in os.environ and 'WORLD_SIZE' in os.environ:
        #RANK 在所有进程中的第几个进程，WORLD_SIZE所有进程数量，LOCAL_RANK当前机器上第几个进程
        # RANK是当前机器的当前进程在所有进程中是第几个进程，LOCAL RANK是当前机器的最后一个进程，就是当前机器的GPU数量
        args.rank = int(os.environ["RANK"])
        args.world_size = int(os.environ['WORLD_SIZE'])
        args.gpu = int(os.environ['LOCAL_RANK'])

        # print(os.environ['LOCAL_RANK'], args.gpu,666666666)
        # print(os.environ)
